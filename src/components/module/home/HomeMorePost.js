  ...
  return (
    <div className="container">
      ...
      <div className="box-chill">
        {post.map((posts) =>{
            let maxLength=23
            let descFormat=posts.desk
            if(posts.desk && posts.desk.length > maxLength){
            descFormat=posts.desk.slice(0, maxLength).concat("...");
            }
            ...
            <BoxImage>
              ...
              <BoxModal className="modal">
                <div className="w-3/4 mx-auto mt-2 text-center">
                  <Link
                    className="font-semibold text-white transition-all hover:text-red-300 text-[14px]"
                    to={posts.slug}
                  >
                    <span className="text-xs">{descFormat}</span>
                  </Link>
                </div>
              </BoxModal>
            </BoxImage>
